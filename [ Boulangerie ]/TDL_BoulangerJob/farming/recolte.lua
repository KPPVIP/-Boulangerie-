...
function OpenboulangerieRecolte()
    local boulangerieRecolte = RageUI.CreateMenu("~u~Récolte blé~s~", "~b~Tablette")
    ...
    while boulangerieRecolte do
        ...
        RageUI.IsVisible(boulangerieRecolte, true, true, true, function()
                RageUI.ButtonWithStyle("Récolte blé", "→   ~g~Récolter du blé~s~", {}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    RageUI.CloseAll()
                    recolteboulangerie()
                    end

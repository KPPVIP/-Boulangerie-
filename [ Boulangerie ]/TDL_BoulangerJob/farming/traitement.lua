...
function OpenboulangerieTraitement()
    local boulangerieTraitement = RageUI.CreateMenu("~u~Traitement~s~", "~b~actions")
    ...
    while boulangerieTraitement do
        ...
        RageUI.IsVisible(boulangerieTraitement, true, true, true, function()
                RageUI.ButtonWithStyle("Traitement farine", "→   ~g~Créer votre jus de raisin", {}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    RageUI.CloseAll()

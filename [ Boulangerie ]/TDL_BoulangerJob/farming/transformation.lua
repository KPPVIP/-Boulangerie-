...
function OpenboulangerieTransformation()
    local boulangerieTransformation = RageUI.CreateMenu("~u~Transformation~s~", "~b~Boulangerie")
    ...
    while boulangerieTransformation do
        ...
        RageUI.IsVisible(boulangerieTransformation, true, true, true, function()
                RageUI.ButtonWithStyle("Transformation pain chaud", "→   ~g~Créer votre pain", {}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    RageUI.CloseAll()

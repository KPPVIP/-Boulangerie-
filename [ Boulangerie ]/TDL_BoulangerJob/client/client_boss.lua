...
function PourBlanchir()
    local TKTPOTO = RageUI.CreateMenu("~u~Compte Offshore~s~", "~b~Luxembourg Exchange System")
...
function Bossboulangerie()
    local Cboulangerie = RageUI.CreateMenu("~u~Patron~s~", "~b~Tablette de l'entreprise")
      ...
      RageUI.Visible(Cboulangerie, not RageUI.Visible(Cboulangerie))
            ...
            if societyboulangeriemoney ~= nil then
                RageUI.ButtonWithStyle("Capital de l'entreprise :", nil, {RightLabel = "$" .. societyboulangeriemoney}, true, function()
                end)
            end

            RageUI.ButtonWithStyle("Retirer argent de société","→   ~r~Retirer de l'argent", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if Selected then
            ...
            RageUI.ButtonWithStyle("Déposer argent de société","→   ~g~Déposer de l'argent", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if Selected then
                    local amount = KeyboardInput("Montant", "", 10)
                    amount = tonumber(amount)
                    if amount == nil then
            ...
            RageUI.ButtonWithStyle("Accéder aux actions de Management","→   ~g~Gestion de l'entreprise", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if Selected then
                    aboss()
                    RageUI.CloseAll()
                end
